...
impl Error {
    ...
    pub fn new(method: &'static str, kind: ErrorKind) -> Self { Self { kind, method: Some(method), errors: None } }

    pub fn check(method: &'static str, hr: HRESULT) -> Result<(), Self> {
        if !SUCCEEDED(hr) {
            Err(Self {
                kind:   ErrorKind::from_hr_allow_ok(hr),
                method: Some(method),
                errors: None,
            })
        } else {
    ...
    pub unsafe fn check_blob(method: &'static str, hr: HRESULT, errors: *mut ID3DBlob) -> Result<(), Self> {
        if !SUCCEEDED(hr) {
