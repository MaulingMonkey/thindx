...
pub struct OptTextBlob(Option<ReadOnlyBlob>);
...
impl Debug   for OptTextBlob { fn fmt(&self, fmt: &mut Formatter) -> fmt::Result { write!(fmt, "OptTextBlob({:?})", self.to_utf8_lossy()) } }
impl Display for OptTextBlob { fn fmt(&self, fmt: &mut Formatter) -> fmt::Result { Display::fmt(&self.to_utf8_lossy(), fmt) } }

impl From<Option<ReadOnlyBlob>> for OptTextBlob { fn from(value: Option<ReadOnlyBlob>) -> Self { Self(     value ) } }
impl From<       ReadOnlyBlob > for OptTextBlob { fn from(value:        ReadOnlyBlob ) -> Self { Self(Some(value)) } }
