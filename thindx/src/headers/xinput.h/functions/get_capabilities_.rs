...
#[test] fn test_bad_arguments() {
    ...
    assert_eq!(ERROR::BAD_ARGUMENTS,        get_capabilities(4u32,      Flag::Gamepad));            // Bad User (obb)
    assert_eq!(ERROR::BAD_ARGUMENTS,        get_capabilities(0u32,      Flag::from_unchecked(42))); // Bad Flag (obb)
    assert_eq!(ERROR::BAD_ARGUMENTS,        get_capabilities(0u32,      Flag::from_unchecked(!0))); // Bad Flag (obb)
}
